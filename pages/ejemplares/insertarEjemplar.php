...
<?php 
	...
	$formejemplarcodigo=$_POST['formejemplarcodigo'];
	$formejemplarcodlib=$_POST['formejemplarcodigolib'];
	$formejemplarestado=$_POST['formejemplarestado'];
	$formejemplarcomentario=$_POST['formejemplarcomentario'];
   ...
   // obtener codigo del ejemplar
    ...
    while ($datacodigo=mysqli_fetch_assoc($consulta)){
    	 while ($datacodigo2=mysqli_fetch_assoc($consulta2)){
          $newcodigo= $ejemplarcodigo."".$datacodigo['dewe']."-".$datacodigo2['codigo'];
      }
    }
...
// 		} else {
		...
		$insRegistro=mysqli_query($conexion,"
		    INSERT INTO  $tablaBitacora(
		      ...
		      NOW(),
		      ' ingreso un nuevo ejemplar: $ejemplarcodigo',
		      '$usuCodigo',
		      '---',
		      '$bitPersonaName');")
		    or die ('ERROR INS-INS:'.mysqli_error($conexion));
